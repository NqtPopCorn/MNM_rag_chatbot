...
================================================
Iterative retrieval: retrieve → evaluate sufficiency → follow-up → repeat.

Change vs. original
───────────────────
The first retrieval query is now contextualized via HistoryManager before
hitting the vector store.  Follow-up queries (generated by the LLM) are
already self-contained by construction, so only the entry query needs this.

Public API:
  CoRAGChain.invoke(question, chat_history) → {"answer": str, "trace": list}
...
class CoRAGChain:
    ...
    prompt_mode    : key of CORAG_FINAL_PROMPTS
    ...
    def _run_iterations(
        ...
        The FIRST query is contextualized (history-aware rewrite) so that
        pronouns and implicit references are resolved before hitting FAISS.
        Subsequent follow-up queries are generated by the LLM and are already
        self-contained, so they skip re-contextualization.
        """
        all_docs: List[Document] = []
        seen:     set            = set()
        trace:    List[dict]     = []

        # ── Contextualize the entry query ────────────────────────────────────
        current_query = self._hm.contextualize(question, chat_history, self.llm)
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            if iteration < self.max_iterations:
                ...
                if not sufficient and follow_up:
                    ...
                    current_query = follow_up   # LLM-generated → already standalone
                trace.append(step)
                if sufficient:
                    break
            else:
