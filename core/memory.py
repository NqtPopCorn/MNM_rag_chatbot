...
Why contextualization matters for RAG:
  A question like "Phương pháp thứ hai có nhược điểm gì?" sent directly to FAISS
  will match chunks containing "phương pháp thứ hai" literally — the pronoun
  reference is lost.  Rewriting it to "Nhược điểm của phương pháp Attention
  pooling trong bài báo XYZ là gì?" gives dramatically better chunk recall.
"""
...
Rules:
- If the follow-up question is already standalone (no pronouns or implicit references \
to prior turns), return it UNCHANGED.
...
# Import this directly: `from core.memory import history_manager`
# All chain types share the same instance (stateless, safe).
